...
# 備份
backup_file = f"venues.json.backup.amazinghall_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
base_url = 'https://www.amazinghall.com.tw/branch_intro/%E5%B3%87%E9%87%8C%E6%96%AF%E8%8A%8D%E5%9C%92/'
    ...
    if '.pdf' in link['href'].lower():
        ...
        if not pdf_url.startswith('http'):
            if pdf_url.startswith('/'):
                pdf_url = 'https://www.amazinghall.com.tw' + pdf_url
...
for link in soup.find_all('a', href=True):
    ...
    # 轉換為絕對URL
    if href.startswith('/'):
        full_url = 'https://www.amazinghall.com.tw' + href
    ...
    # 只保留內部連結
    if 'amazinghall.com.tw' in full_url and full_url not in seen_urls:
...
try:
    r_home = requests.get('https://www.amazinghall.com.tw/', timeout=15, verify=False, headers=headers)

...
for link in soup.find_all('a', href=True):
    ...
    # 尋找包含會議/宴會/活動等關鍵字的連結
    keywords = ['meeting', 'banquet', 'conference', 'event', 'mice', 'wedding', '會議', '宴會', '會議室', '婚宴', '圓山']
...
if venue:
    ...
    rooms_with_data = sum(1 for r in rooms if r.get('capacity') or r.get('area'))
    print(f"Rooms with data: {rooms_with_data}/{len(rooms)}")

    for room in rooms:
        print(f"  - {room.get('name')}: capacity={room.get('capacity')}, area={room.get('area')}")
...
with open('grandhotel_stage1_results.json', 'w', encoding='utf-8') as f:
...
print(f"Result saved: grandhotel_stage1_results.json")

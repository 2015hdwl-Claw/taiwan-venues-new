...
def extract_room_code(url):
    ...
    if 'the-lecture-hall' in url:
        return 'lecture_hall'
    return None

url_to_data = {}
for data in stage2['rooms']:
    room_code = extract_room_code(data['url'])
    if room_code:
        url_to_data[room_code] = data
...
for room in venue.get('rooms', []):
    ...
    # 從 room_id 中提取代碼（例如：1495-room_201 → room_201）
    room_code = None
    if 'room_' in room_id:
        room_code = room_id.split('-')[-1]

    if room_code not in url_to_data:
        print(f"\n⚠️  {room_id}: 無階段2資料")
        continue
    ...
    print(f"\n{room_id} ({correct_name}):")

    # 更新名稱
    if room['name'] != correct_name:
        print(f"  名稱: {room['name']} → {correct_name}")
        room['name'] = correct_name
    ...
    # 驗證並更新價格
    if correct_name in price_corrections:
        correct_price = price_corrections[correct_name]

...
for a in soup.find_all('a', href=True):
    ...
    if any(kw in text.lower() or kw in href.lower() for kw in
           ['會議', '場地', '空間', '場地租借', '會議室', 'room', 'space', 'venue']):
        # 轉換為完整 URL
...
for i, link in enumerate(unique_links[:10], 1):  # 限制測試前10個
    print(f"\n測試 {i}/{min(10, len(unique_links))}: {link['text'][:50]}")
    ...
    try:
        ...
        if test_response.status_code == 200:
            ...
            # 檢查是否包含場地資訊
            has_data = any(kw in page_text for kw in ['坪', '人', '容量', '面積', '租金', '費用'])
...
if len(unique_rooms) == 0:
    ...
    print(f"  ⚠️  會議室資料較少")
    print(f"  建議：手動檢查是否有其他隱藏的會議室頁面")
else:
    print(f"  ✅ 找到 {len(unique_rooms)} 個會議室")
    print(f"  建議：進入階段3，提取完整 30 欄位資料")

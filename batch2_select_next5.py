...
# 排序：優先 A 級，然後 B 級，最後 C 級
unverified.sort(key=lambda x: (
    -1 if x['grade'] == 'A' else 1 if x['grade'] == 'B' else 2,
    -x['total_score']
))

print(f"📊 未驗證場地統計:")
print(f"   總數: {len(unverified)} 家")
print(f"   A 級: {sum(1 for v in unverified if v['grade'] == 'A')} 家")
print(f"   B 級: {sum(1 for v in unverified if v['grade'] == 'B')} 家")
print(f"   C 級: {sum(1 for v in unverified if v['grade'] == 'C')} 家")
print()
...
for venue in unverified[:15]:
    print(f"{venue['grade']:<6} {venue['total_score']:<6} {venue['image_pct']:.0f}%{' ':<6} {venue['price_pct']:.0f}%{' ':<6} {venue['rooms']:<8} {venue['name'][:40]}")

print()

# 選擇下一批 5 家
next_batch = unverified[:5]
...
for i, venue in enumerate(next_batch, 1):
    print(f"{i}. {venue['name']} (ID: {venue['id']})")
    print(f"   等級: {venue['grade']} ({venue['total_score']}%)")
    print(f"   會議室: {venue['rooms']} 間")
    print(f"   照片: {venue['image_pct']:.0f}%, 價格: {venue['price_pct']:.0f}%")
    print(f"   官網: {venue['url']}")
    print()
...
# Create backup
backup_path = f"venues.json.backup.batch2_{{datetime.now().strftime('%Y%m%d_%H%M%S')}}"
...
for venue in next_batch:
    script_content += f"    {{'id': {venue['id']}, 'name': '{venue['name']}'}},\n"
...
print(f"   3. 驗證價格和照片來源")
print(f"   4. 標記需要修正的項目")
print(f"   5. 繼續處理剩餘 {{len(data) - 10}} 家飯店")
'''

# 寫入腳本
script_path = 'batch2_verify_5hotels.py'
...
print(f"選擇的場地:")
for i, venue in enumerate(next_batch, 1):
    print(f"  {i}. {venue['name']} (ID: {venue['id']}) - {venue['grade']} 級")
print()

...
# Create backup
backup_path = f"venues.json.backup.batch3_10hotels_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
verified_ids = [
    # 批次 1
    1043, 1076, 1077, 1103, 1072,
    # 批次 2
    1090, 1075, 1122, 1069, 1085
]
...
# 選擇接下來的 10 家
next_batch = unverified_hotels[:10]

print(f"📊 未驗證飯店統計:")
print(f"   總數: {len(unverified_hotels)} 家")
print(f"   本次選擇: 10 家")
print(f"   剩餘: {len(unverified_hotels) - 10} 家")
print()
...
# 執行批次驗證
hotels = [{'id': h['id'], 'name': h['name']} for h in next_batch]
...
print(f"\n{'='*120}")
print("✅ 批次 3：10 家飯店資料已準備好供人工檢查")
print(f"{'='*120}")
print(f"\n備份檔案: {backup_path}")
print(f"更新時間: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

print(f"\n💡 下一步:")
print(f"   1. 用戶檢查批次 2（5 家）")
print(f"   2. 用戶檢查批次 3（10 家）")
print(f"   3. 繼續處理剩餘 {len(unverified_hotels) - 10} 家飯店")

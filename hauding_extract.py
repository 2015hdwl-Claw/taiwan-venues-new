...
# 備份
backup_file = f"venues.json.backup.hauding_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
base_url = 'https://www.how-dine.com.tw/'
...
for link in soup.find_all('a', href=True):
    ...
    # 只保留內部連結
    if 'how-dine.com.tw' in full_url and full_url not in seen_urls:
...
for i, link in enumerate(meeting_links[:5]):
    url = link['url']
    print(f"\n[{i+1}] {link['text']}")
    print(f"URL: {url}")
    print("-" * 80)
    ...
    try:
        ...
        if r2.status_code == 200:
            ...
            # 提取會議室
            rooms2 = re.findall(r'([^\s]{2,10}[廳室房])(?:\s|[,，.。、\n])', page_text2)
            if rooms2:
                print(f"  會議室: {rooms2[:10]}")

            # 提取容量
            capacities2 = re.findall(r'(\d+)\s*[人名桌者席位]', page_text2)
            if capacities2:
                caps2 = [int(c) for c in capacities2 if 5 <= int(c) <= 1000]
                print(f"  容量: {caps2[:10]}")

            # 提取面積
            areas2 = re.findall(r'(\d+|\d+\.\d+)\s*([坪平方公尺㎡㎡²])', page_text2)
            if areas2:
                print(f"  面積: {areas2[:10]}")

            # 提取價格
            prices2 = re.findall(r'(\d+,?\d*)\s*元', page_text2)
            if prices2:
                print(f"  價格: {prices2[:10]}")

            # 尋找PDF
            pdf_links2 = []
            for link2 in soup2.find_all('a', href=True):
                if '.pdf' in link2['href'].lower():
                    pdf_url2 = link2['href']
                    if not pdf_url2.startswith('http'):
                        if pdf_url2.startswith('/'):
                            pdf_url2 = base_url.rstrip('/') + pdf_url2
                    pdf_links2.append(pdf_url2)

            if pdf_links2:
                print(f"  PDF: {pdf_links2}")
...
# 更新基本聯絡資訊
venue['contact']['phone'] = '+886-2-2956-7868'
venue['contact']['email'] = 'service@how-dine.com.tw'

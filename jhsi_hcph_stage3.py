...
if len(stage2['stage2']['pdf_links']) > 0:
    print(f"  PDF: {len(stage2['stage2']['pdf_links'])} 個")
    for pdf in stage2['stage2']['pdf_links']:
        print(f"    - {pdf['text']}")
        print(f"      {pdf['href']}")

print("\n發現的會議室:")
# 過濾出只屬於 HSPH 的會議室
hsph_rooms = ['巴哈廳', '羅西尼廳', '鄧肯廳', '愛因斯坦廳', '愛迪生廳']
print(f"  集思竹科專屬會議室: {len(hsph_rooms)} 個")
for room in hsph_rooms:
    print(f"    - {room}")
...
print("\n【資料來源】")
print(f"  官網有基本會議室名稱: {', '.join(hsph_rooms)}")
print(f"  缺少詳細資料: 容量、面積、價格、設備")
print(f"  PDF 連結是課程表，非場地資料")
...
print("\n【下一步建議】")
print(f"  選項 1: 聯繫集思竹科會議中心索取完整場地資料")
print(f"  選項 2: 查詢其他集思場地資料作為參考（集思台大已有完整資料）")
print(f"  選項 3: 手動從官網提取各會議室的詳細資訊")

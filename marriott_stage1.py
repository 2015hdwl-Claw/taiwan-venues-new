...
for link in soup.find_all('a', href=True):
    ...
    # 尋找包含會議/宴會/活動等關鍵字的連結
    keywords = ['meeting', 'banquet', 'conference', 'event', 'mice', 'wedding', '會議', '宴會', '會議室', '婚宴', '萬豪']
...
if venue:
    ...
    for room in rooms[:3]:
        ...
        price = room.get('price', {}).get('weekday', 'N/A')
        print(f"  - {name}: cap={capacity}, area={area}㎡, price={price}")
...
with open('marriott_stage1_results.json', 'w', encoding='utf-8') as f:
...
print(f"Result saved: marriott_stage1_results.json")

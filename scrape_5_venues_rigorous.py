...
def stage1_technical_detection(url):
    ...
    # 檢查 HTML 結構
    meeting_keywords = ['meeting', 'conference', 'room', '會議', '場地', '會議室', '宴會', '婚宴']
    found_keywords = []
...
def stage2_deep_scraping(base_url, soup, response):
    ...
    for a in all_links:
        ...
        if any(kw in href_lower or kw in text for kw in
               ['meet', 'mice', 'banquet', 'event', 'conference', 'wedding',
                '會議', '宴會', '活動', '場地', '會議室', '聯絡', '婚宴', '空間', '租借']):
            # 排除無效連結
            if not href_lower.startswith('mailto:') and not href_lower.startswith('tel:') and not href_lower.startswith('javascript:'):
    ...
    for a in all_links:
        href = a['href'].lower()
        text = a.get_text()
        if '.pdf' in href or 'download' in href or any(kw in text for kw in ['價目', '收費', '租用', '場租', '費率']):
    ...
    print(f"    PDF/價格連結: {len(pdf_links)} 個")
    ...
    if found_rooms:
        ...
        print(f"    {result['potential_rooms'][:15]}")
...
if __name__ == '__main__':
    ...
    venues = [
        ...
        {'id': 1125, 'name': '華山1914文化創意產業園區', 'url': 'https://www.huashan1914.com'},
    ...
    for venue in venues:
        result = scrape_venue_rigorous(venue['id'], venue['name'], venue['url'])
        if result:
    ...
    # 儲存所有結果
    with open(f'scrape_5_venues_rigorous_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json',
              'w', encoding='utf-8') as f:
        json.dump(all_results, f, ensure_ascii=False, indent=2)

    print(f"\n\n{'=' * 100}")
    print(f"✅ 所有場地檢測完成，結果已儲存")
    print(f"{'=' * 100}")
